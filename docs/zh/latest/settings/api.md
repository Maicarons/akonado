...
- **items: Array[KonadoSettingItem]**：分类包含的设置项数组
...
设置值会自动保存到 `user://konado_settings.cfg` 文件中，使用 Godot 的 `ConfigFile` 格式。
...
1. **添加新设置类型**：
   - 在 `KonadoSettingItem.Type` 中添加新枚举值
