...
업적 시스템은 JSON 설정 파일로 업적을 정의합니다. 기본 경로는 `res://addons/konado_achievement/data/default_achievements.json`이며, `KonadoAchievements`에서 다른 경로를 설정할 수 있습니다.
...
`KonadoAchievements`에서 다음 속성을 설정할 수 있습니다.
