...
- **items: Array[KonadoSettingItem]**：分類包含的設定項目陣列。
...
設定值會使用 Godot 的 `ConfigFile` 格式，自動儲存到 `user://konado_settings.cfg` 檔案中。
...
1. **新增設定類型**：
   - 在 `KonadoSettingItem.Type` 中新增列舉值。
