...
- **items: Array[KonadoSettingItem]**: カテゴリに含まれる設定項目の配列。
...
設定値は Godot の `ConfigFile` 形式で、`user://konado_settings.cfg` に自動保存されます。
...
1. **新しい設定タイプを追加する**:
   - `KonadoSettingItem.Type` に新しい列挙値を追加します。
