...
実績システムは JSON 設定ファイルで実績を定義します。デフォルトのパスは `res://addons/konado_achievement/data/default_achievements.json` で、`KonadoAchievements` から別のパスを設定できます。
...
`KonadoAchievements` では、以下のプロパティを設定できます。
