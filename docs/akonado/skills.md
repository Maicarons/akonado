...
以下约束由 Konado 解析器的实现决定，违反任何一条都会导致运行时解析失败：

| 约束 | 原因 |
|------|------|
| **branch 内容必须缩进 4 个空格** | `ks_interpreter.gd` 的 `_parse_indented_block()` 通过缩进判断 branch 边界 |
| **所有引号必须使用 ASCII 直引号 `"` (0x22)** | 对话正则 `^"(.*?)"\s+"(.*?)"` 只匹配 ASCII 引号，Unicode 弯引号 `""` 会导致"无法识别的语法" |
| **LF 换行符** | 混合 CRLF/LF 会导致解析器行号偏移 |
| **branch 内禁止 `actor show`** | 解释器解析 branch 时共享外部角色状态，会导致"角色已存在"错误 |
| **branch 内禁止 `actor exit`** | ending 分支中解释器会自动清理角色状态，手动 exit 会导致"无法移除不存在的角色" |
| **同一角色不能重复 `show`** | 必须先 `actor exit` 再 `actor show` |
| **`actor move` 不能移动到当前位置** | 会导致信号不触发、状态机卡死 |
| **`if/else/endif` 不支持嵌套** | 使用扁平结构 |
| **ending 分支只需要旁白、背景、BGM、end** | 不要加 `actor exit`，解释器自动清理 |

branch 内容缩进示例：

```
branch my_branch
    "chenmo" "对话内容" voice_tag
    "narrator" "旁白内容"
    actor change chenmo sad
    jump_branch ending
```

这些约束已内置在 `generate_scene_script` 的 system_prompt 中，LLM 会自动遵守。如需手动编辑 .ks 文件，请务必参照以上规范。
