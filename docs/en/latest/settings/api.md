...
## Settings Manager (KonadoSettings)
...
#### `register_category(cat: KonadoSettingCategory) -> void`
...
- **items: Array[KonadoSettingItem]**: Array of setting items contained in the category.

### Example

```gdscript
var video_cat = KonadoSettingCategory.new()
video_cat.id = "video"
...
# Register category
KonadoSettings.register_category(video_cat)
```

## Setting Item (KonadoSettingItem)
...
```gdscript
var volume_item = KonadoSettingItem.new()
volume_item.key = "master_volume"
volume_item.label = "Master Volume"
volume_item.type = KonadoSettingItem.Type.SLIDER
...
```gdscript
var fullscreen_item = KonadoSettingItem.new()
fullscreen_item.key = "fullscreen"
fullscreen_item.label = "Fullscreen"
fullscreen_item.type = KonadoSettingItem.Type.TOGGLE
...
```gdscript
var language_item = KonadoSettingItem.new()
language_item.key = "language"
language_item.label = "Language"
language_item.type = KonadoSettingItem.Type.OPTION
...
#### `create_control(cat_id: String, item: KonadoSettingItem, callback: Callable) -> HBoxContainer`
...
var row = UIFactory.create_control("audio", volume_item, func(category, key, value):
	...
	KonadoSettings.set_setting(category, key, value)
)
...
var settings_panel = preload("res://addons/konado_settings/ui/konado_settings_panel.tscn").instantiate()
...
KonadoSettings.register_category(new_category)
...
Setting values are automatically saved to `user://konado_settings.cfg` using Godot's `ConfigFile` format.
...
```gdscript
# Get a setting value
var master_volume = KonadoSettings.get_setting("audio", "master_volume")

# Set a value
KonadoSettings.set_setting("audio", "master_volume", 0.8)

# Listen for setting changes
KonadoSettings.setting_changed.connect(func(category, key, value):
...
# Create setting item
var resolution_item = KonadoSettingItem.new()
resolution_item.key = "resolution"
resolution_item.label = "Resolution"
resolution_item.type = KonadoSettingItem.Type.OPTION
...
1. **Add a new setting type**:
   - Add a new enum value to `KonadoSettingItem.Type`.
