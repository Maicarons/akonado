@tool
extends Control
class_name KonadoTypewriterText
		...
		. push_back(
			{
				...
				"horizontal_position": Vector2.ZERO,
				"advance": 0.0,
			}
		)
	)
...
func _init_shader() -> void:
	var TypewriterFadeShader := preload(
		"res://addons/konado/runtime/ui/typewriter/typewriter_fade.gdshader"
	)
