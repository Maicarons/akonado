...
# Usage: scripts/Linux/pipeline.sh "a story about a tea shop"
cd "$(dirname "$0")/../.."
if [ $# -eq 0 ]; then
    echo "Usage: scripts/Linux/pipeline.sh \"your story premise\""
    exit 1
fi
PREMISE="$1"
shift
...
if [ $? -ne 0 ]; then
    ...
    echo ""
    echo "[akonado] Pipeline complete! Run 'scripts/Linux/godot.sh' to open in Godot."
fi
